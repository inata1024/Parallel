...
int main()
{
    ...
    {
        CACHE_ALIGN int C_raw = fileSize[tms].a, R1 = fileSize[tms].b, R2 = fileSize[tms].c;//原始矩阵列数,非零消元子,被消元行,

        int C = ceil(C_raw / 32.0);//位向量矩阵列数
        //为消元子、被消元行分配空间
        int** a = (int**)_aligned_malloc(R1*sizeof(int*),64);
        int** b = (int**)_aligned_malloc(R2*sizeof(int*),64);
        int* ini = (int*)_aligned_malloc(C_raw*sizeof(int),64);
        ...
        for (int i = 0; i < C_raw; i++)
            ini[i] = -1;
        for (int i = 0; i < R1; i++)
            a[i] = (int*)_aligned_malloc(C*sizeof(int),64);
        //a[i] = new int[C] {0};
        for (int i = 0; i < R2; i++)
            b[i] = (int*)_aligned_malloc(C*sizeof(int),64);
        //b[i] = new int[C] {0};
        //读入数据集并构建矩阵
