//x86特殊高斯消去 pthread 静态线程 mutex加锁 AVX并行
#include <stdlib.h>
#include <windows.h>
#include<pthread.h>
#include <iostream>
        ...
        for (int c = C - 1;c >= 0;c--)//每一列位向量
        {
            ...
            while (b[r][c] != 0 && !end)//从后往前消去
            {
                ...
                if (temp > -1)//若存在消元行
                {
                    ...
                    for (i = 0;i < C - 8;i += 8)
                    {
                        vbri = _mm256_loadu_si256((__m256i*)&b[r][i]);
                        ver = _mm256_loadu_si256((__m256i*)&er[i]);
                        vbri = _mm256_xor_si256(vbri, ver);
                        _mm256_storeu_si256((__m256i*)& b[r][i], vbri);
                    }
                    //串行处理剩下的
                    for (;i < C;i++)
                        b[r][i] ^= er[i];
