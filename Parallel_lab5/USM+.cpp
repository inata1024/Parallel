...
// =============================================================
//普通高斯消去GPU Unified Shared Memory Plus
...
int main()
{
    ...
    for(int tms=0;tms<1;tms++)
    {
        ...
        auto div=malloc_device<float>(N,q);//显式内存分配，用于除法操作
        for(int i=0;i<N;i++)
        {
            //m[i]=new float[N];
        ...
        for(int k=0;k<N;k++)
        {
            //卸载除法操作到device
            //显式向设备拷贝内存
            q.memcpy(div,m[k],sizeof(float)*N);
            //设备端修改数据
            q.parallel_for(N-k-1,[=](auto i){
                div[k][i+k+1]=div[k][i+k+1]/div[k][k];
            }).wait();
            //显式向主机拷贝内存
            q.memcpy(m[k],div,sizeof(float)*N);
            //for(int j=k+1;j<N;j++)
            //    m[k][j]=m[k][j]/m[k][k];
            m[k][k]=1.0;
